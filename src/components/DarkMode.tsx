...
function DarkMode() {
    const [darkMode, setRecoilDarkMode] = useRecoilState(recoilDarkMode),
    ...
    function setDarkMode(newDarkMode:boolean) {
        localStorage.setItem('darkMode', JSON.stringify({'value': newDarkMode}));
        setRecoilDarkMode(newDarkMode);
    }
    return (
